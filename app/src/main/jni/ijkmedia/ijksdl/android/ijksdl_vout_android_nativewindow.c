...
extern void CyExternalRenderFunc(void* obj, int width, int height,
...
static int func_display_overlay_l(SDL_Vout *vout, SDL_VoutOverlay *overlay)
{
    ...
    void* external_renderer = opaque->external_renderer; 
	//__android_log_print(ANDROID_LOG_DEBUG, "x4x", "func_display_overlay_l"); 
    ...
    if(external_renderer) {
        //ALOGD("w %d, h %d", overlay->w, overlay->h); 
        //__android_log_print(ANDROID_LOG_DEBUG, "x4x", "EXTERNAL"); 
        ...
        CyExternalRenderFunc(external_renderer, overlay->w, overlay->h, 
            pixels[0], widths[0], pixels[1], widths[1], pixels[2], widths[2]); 
        return 0; 
    }
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "x4x", "x1");
    switch(overlay->format) {
    case SDL_FCC__AMC: {
        //__android_log_print(ANDROID_LOG_DEBUG, "x4x", "x2");
    ...
    case SDL_FCC_I444P10LE: {
        //__android_log_print(ANDROID_LOG_DEBUG, "x4x", "x3");
        // only GLES support
        if (opaque->egl)
    ...
    case SDL_FCC_RV16:
    case SDL_FCC_RV32: {
        //__android_log_print(ANDROID_LOG_DEBUG, "x4x", "x4");
...
static void SDL_VoutAndroid_SetAndroidExternalRenderer_l(SDL_Vout *vout, void* ext_render)
{
    AMCTRACE("%s(%p, %p)\n", __func__, vout, ext_render);
    SDL_Vout_Opaque *opaque = vout->opaque;
    if (opaque->external_renderer == ext_render) {
        if (ext_render == NULL) {
            // always invalidate buffers, if native_window is changed
            SDL_VoutAndroid_invalidateAllBuffers_l(vout);
        }
    ...
    opaque->external_renderer = ext_render; 
    opaque->native_window = NULL;
    opaque->null_native_window_warned = 0;
}

void SDL_VoutAndroid_SetAndroidExternalRenderer(SDL_Vout *vout, void* ext_render)
{
    SDL_LockMutex(vout->mutex);
    SDL_VoutAndroid_SetAndroidExternalRenderer_l(vout, ext_render);
    SDL_UnlockMutex(vout->mutex);
}
