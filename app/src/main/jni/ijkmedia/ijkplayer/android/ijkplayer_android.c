...
void ijkmp_android_set_external_renderer_l(JNIEnv *env, IjkMediaPlayer *mp, jlong ext_render)
{
    if (!mp || !mp->ffplayer || !mp->ffplayer->vout)
        return;
    void* ptr = (void*)ext_render;
    SDL_VoutAndroid_SetAndroidExternalRenderer(mp->ffplayer->vout, ptr);
    ffpipeline_set_surface(env, mp->ffplayer->pipeline, NULL);
}

void ijkmp_android_set_external_renderer(JNIEnv *env, IjkMediaPlayer *mp, jlong ext_render)
{
    if (!mp)
        return;
    void* ptr = (void*)ext_render;
    MPTRACE("ijkmp_android_set_external_renderer(surface=%p)", ptr);
    pthread_mutex_lock(&mp->mutex);
    ijkmp_android_set_external_renderer_l(env, mp, ext_render);
    pthread_mutex_unlock(&mp->mutex);
    MPTRACE("ijkmp_android_set_external_renderer(ext_render=%p)=void", ptr);
}
